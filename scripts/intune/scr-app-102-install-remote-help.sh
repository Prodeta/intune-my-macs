...
updateMAU () {
    ...
    if [[ $? == 0 ]]; then
        ...
        echo "$(date -u "+%Y-%m-%d %H:%M:%S UTC") | Failure to download [MAU]"
        exit 1
    fi
}
...
downloadApp () {
    ...
    if [[ $? == 0 ]]; then
        echo "$(date -u "+%Y-%m-%d %H:%M:%S UTC") | Downloaded to [$tempfile]"
        case $tempfile in
            *.pkg|*.PKG|*.mpkg|*.MPKG)
                packageType="PKG"
                ;;
        ...
        echo "$(date -u "+%Y-%m-%d %H:%M:%S UTC") | Failure to download [$weburl]"
        updateOctory failed
        exit 1
    fi
}
