...
install_dockutil_if_missing() {
  ...
  if [[ ! -e "/usr/local/bin/dockutil" ]]; then
    echo "$(date -u "+%Y-%m-%d %H:%M:%S UTC") | dockutil is not present, installing"
    ...
    if curl -L "https://github.com/kcrawford/dockutil/releases/download/3.1.3/dockutil-3.1.3.pkg" -o /var/tmp/dockutil.pkg; then
      echo "$(date -u "+%Y-%m-%d %H:%M:%S UTC") | Successfully downloaded dockutil.pkg"
    else
      echo "$(date -u "+%Y-%m-%d %H:%M:%S UTC") | Failed to download dockutil.pkg"
      return 1
    fi

    # Install dockutil package
    if sudo installer -pkg /var/tmp/dockutil.pkg -target /; then
      echo "$(date -u "+%Y-%m-%d %H:%M:%S UTC") | Successfully installed dockutil"
    else
      echo "$(date -u "+%Y-%m-%d %H:%M:%S UTC") | Failed to install dockutil"
      return 1
    fi

  else
    echo "$(date -u "+%Y-%m-%d %H:%M:%S UTC") | dockutil is already present"
  fi
}
...
function update_swift_dialog () {
    if [[ -e "/Library/Application Support/Dialog/Dialog.app/Contents/MacOS/Dialog" ]]; then
        ...
        echo "$(date -u "+%Y-%m-%d %H:%M:%S UTC") |     Updating Swift Dialog monitor for Dock to [$1]"
...
configure_dock_with_dockutil () {
    ...
    for app in "${dockapps[@]}"; do
      if [[ -e "$app" ]]; then
        echo "$(date -u "+%Y-%m-%d %H:%M:%S UTC") | Adding ${app} to Dock..."
        run_as_user "${dockutil}" --add "${app}" --section apps --no-restart ${plist} > /dev/null 2>&1
      else
        echo "$(date -u "+%Y-%m-%d %H:%M:%S UTC") | ${app} not installed, skipping..."
      fi
    done
...
configure_dock_via_plist () {
  ...
  # Clearing Dock
  echo "$(date -u "+%Y-%m-%d %H:%M:%S UTC") |  Removing Dock Persistent Apps"
  ...
  echo "$(date -u "+%Y-%m-%d %H:%M:%S UTC") |  Adding Apps to Dock"
  for i in "${dockapps[@]}"; do
    if [[ -a "$i" ]] ; then
      echo "$(date -u "+%Y-%m-%d %H:%M:%S UTC") |  Adding [$i] to Dock"
  ...
  echo "$(date -u "+%Y-%m-%d %H:%M:%S UTC") | Adding Downloads Stack"
  ...
  #echo "$(date -u "+%Y-%m-%d %H:%M:%S UTC") | Enabling Magnification"
  #defaults write com.apple.dock magnification -boolean YES

  #echo "$(date -u "+%Y-%m-%d %H:%M:%S UTC") | Enable Dim Hidden Apps in Dock"
  #defaults write com.apple.dock showhidden -bool true

  #echo "$(date -u "+%Y-%m-%d %H:%M:%S UTC") | Enable Auto Hide dock"
  #defaults write com.apple.dock autohide -bool true

  #echo "$(date -u "+%Y-%m-%d %H:%M:%S UTC") | Disable show recent items"
  #defaults write com.apple.dock show-recents -bool FALSE

  #echo "$(date -u "+%Y-%m-%d %H:%M:%S UTC") | Enable Minimise Icons into Dock Icons"
  #defaults write com.apple.dock minimize-to-application -bool yes

  echo "$(date -u "+%Y-%m-%d %H:%M:%S UTC") | Restarting Dock"
  killall -KILL Dock
...
wait_for_apps_installation() {
    ...
    while true; do
        ...
        if [[ "$all_installed" = true ]]; then
            echo "$(date -u "+%Y-%m-%d %H:%M:%S UTC") | All apps are installed"
            return 0
        fi
        ...
        if [[ $elapsed_time -ge $timeout ]]; then
            echo "$(date -u "+%Y-%m-%d %H:%M:%S UTC") | Timeout reached. Not all apps are installed."
            return 1
        fi
...
if [[ "$waitForApps" == true ]]; then
    echo "$(date -u "+%Y-%m-%d %H:%M:%S UTC") | Waiting for apps to be installed..."
...
if [[ "$useDockUtil" == true ]]; then
    echo "$(date -u "+%Y-%m-%d %H:%M:%S UTC") | Configuring dock with dockutil"
    install_dockutil_if_missing
    configure_dock_with_dockutil
else
    echo "$(date -u "+%Y-%m-%d %H:%M:%S UTC") | Configuring dock with plist"
    configure_dock_via_plist
fi