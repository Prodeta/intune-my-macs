...
You will get a file called `GatewayWindowsDefenderATPOnboardingPackage.zip`. Extract the archive and, in the Intune folder, rename `WindowsDefenderATPOnboarding.xml` to `cfg-mde-001-onboarding.mobileconfig`.

Copy `cfg-mde-001-onboarding.mobileconfig` into `macOS/mde` in this project.
...
Create `macOS/mde/cfg-mde-001-onboarding.xml` with the following content. The `<SourceFile>` value must match your onboarding payload name and path.
...
<MacIntuneManifest>
  ...
  <SourceFile>macOS/mde/cfg-mde-001-onboarding.mobileconfig</SourceFile>
</MacIntuneManifest>
```

At this point you should have:

- `macOS/mde/cfg-mde-001-onboarding.mobileconfig` – your tenant-specific onboarding payload.
- `macOS/mde/cfg-mde-001-onboarding.xml` – the manifest above.
...
- **Onboarding file not found**  
  ...
  - Confirm it is in the `macOS/mde/` folder.  
